...
The code is use get the predicted values after we have created the model. The `.get_prediction() method is used to get the predicted values based on the datatime.
```{python}
...
The graph allows us to confirm that the testing data and training data are overlapping. This means that the forecasting model would perform well.
```{python}
...
The next step is to create a separate DataFrame which would be helpful to compare the true test data and the predicted values by using the Mean Error Estimation
```{python}
...
We can now approaching the end of the model, to forecast or predict the next 7 values. The variable result contains actual model information. The `.get_forecast()` medthod is responsible to take the information about the model from the variable results, and then
based up the observations of the various patterns it would generate the required forecast value.
```{python}
...
This is it!, the final part is to create the actual plot which would then make all sense of this complex project.
As you can see, the observed and the forecasted values in the plot, the `blue` line is the observed values.
The orange line represents the values which are predicting using SARIMAX Time Series Model.
The shaded region tells that is is whithin 95% confidence interval (CI)
