...
## We convert our date column to datetime object
```{python}
#| label: convert-dt
...
The data range is from `{python} date_min` to `{python} date_max`.
...
The date column is a proper pandas.core.series.Series object.
...
```
We have addressed Missing Values (NaN) through Imputation techniques using the mean() method.
...
We have generated a complete statistical summary with Sweetviz Python package.

# Advanced Exploratory Data Analysis
We're employed the ydata-profiling package, a powerful Time Series Analysis EDA package that offers more detailed analysis than Sweetviz.
